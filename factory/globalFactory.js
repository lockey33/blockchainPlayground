...
export default class globalFactory {
    constructor(mode, accountConfig) {
        ...
        this.helper = new HelperFactory(this.config, this.contractManager)
        this.contractManager = new ContractFactory(this.config)
        this.contracts = this.initContracts(this.config.router, PANCAKE, this.config.provider)
        this.accountManager = new AccountFactory(this.config)
        this.swap = new SwapFactory(this.config)
