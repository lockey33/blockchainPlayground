...
var testNet = "https://bsc-testnet.web3api.com/v1/YG5ZXZX9AX6TA9NZAEX71SR8FAAFYPSCVX";
var publicTestNet = "https://data-seed-prebsc-1-s1.binance.org:8545/";

var mainNet = "https://bsc-mainnet.web3api.com/v1/YG5ZXZX9AX6TA9NZAEX71SR8FAAFYPSCVX";
var mainPublicSocket = "wss://bsc-ws-node.nariox.org:443";
var mainNetHttp = "https://bsc-dataseed.binance.org/";
var localMainNet = "http://localhost:8545";

var _require = require('telegram'),
    TelegramClient = _require.TelegramClient;

var _require2 = require('telegram/sessions'),
    StringSession = _require2.StringSession;

var input = require('input'); // npm i input

var apiId = 4792862;
var apiHash = '1db38acb41a7c8794aaf203564281b12';

var stringSession = new StringSession(''); // fill this later with the value from session.save()
(async function () {
    console.log('Loading interactive example...');
    var client = new TelegramClient(stringSession, apiId, apiHash, { connectionRetries: 5 });
    await client.start({
        ...
        phoneCode: async function phoneCode() {
...
async function swapTokens(tokenIn, tokenOut, amount, gas) {
    ...
    try {
        ...
        var gasPrice = ethers.utils.parseUnits(gas.toString(), "gwei");
...
async function sellTokenForBNB(amountIn, amountOutMin, tokenIn, tokenOut, recipient, gasLimit, gas) {
    var tx = await router.swapExactTokensForETHSupportingFeeOnTransferTokens(amountIn, 0, [tokenIn, tokenOut], recipient, Date.now() + 1000 * 60 * 10, //10 minutes
    {
        gasLimit: gasLimit,
...
async function buyTokenWithBNB(amountIn, amountOutMin, tokenIn, tokenOut, recipient, gasLimit, gas) {
    var tx = await router.swapExactETHForTokens(0, [tokenIn, tokenOut], recipient, Date.now() + 1000 * 60 * 10, //10 minutes
    {
        value: amountIn,
        gasLimit: gasLimit,
        gasPrice: ethers.utils.parseUnits(gas.toString(), "gwei")
